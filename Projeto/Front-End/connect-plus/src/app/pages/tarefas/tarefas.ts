...
type FiltroConclusao = 'todas' | 'somente-concluidas' | 'ocultar-concluidas';
...
export class Tarefas implements OnInit {
  ...
  filtroPesquisa = '';
  filtroDificuldade: DificuldadeTarefa | '' = '';
  ...
  statusFiltros = [
    {
      valor: 'pendente' as StatusTarefa,
      titulo: 'Backlog',
    },
    {
      valor: 'em_andamento' as StatusTarefa,
      titulo: 'Em andamento',
    },
    {
      valor: 'em_revisao' as StatusTarefa,
      titulo: 'Em revisão',
    },
    {
      valor: 'concluida' as StatusTarefa,
      titulo: 'Concluída',
    },
    {
      valor: 'cancelada' as StatusTarefa,
      titulo: 'Cancelada',
    },
    {
      valor: 'arquivada' as StatusTarefa,
      titulo: 'Arquivada',
    },
  ];
  ...
  get responsaveisDoProjetoSelecionado(): ResponsavelOption[] {
    ...
    const idsResponsaveisComTarefaNoProjeto = new Set(
      this.tarefas
        .filter((tarefa) => tarefa.idProjeto === this.filtroProjetoId)
        .map((tarefa) => tarefa.idResponsavelUsuarioEmpresa)
        .filter((idResponsavel) => idResponsavel !== null && idResponsavel !== undefined),
    );

    return this.responsaveis.filter((responsavel) => {
      const vinculadoAoProjeto = responsavel.idProjetos?.includes(this.filtroProjetoId as number);
      const temTarefaNoProjeto = idsResponsaveisComTarefaNoProjeto.has(responsavel.id);

      return vinculadoAoProjeto || temTarefaNoProjeto;
    });
  }
  ...
  get possuiFiltrosAtivos(): boolean {
    return (
      ...
      !!this.filtroPesquisa.trim() ||
      !!this.filtroDificuldade ||
      !!this.filtroPrioridade ||
      !!this.filtroStatus ||
      this.filtroConclusao !== 'todas'
    );
  }

  aoSelecionarProjeto(): void {
    if (!this.filtroResponsavelId) {
      return;
    }

    const responsavelExisteNoProjeto = this.responsaveisDoProjetoSelecionado.some(
      (responsavel) => responsavel.id === this.filtroResponsavelId,
    );

    if (!responsavelExisteNoProjeto) {
      this.filtroResponsavelId = null;
    }
  }

  alternarFiltroResponsavel(idResponsavel: number): void {
    this.filtroResponsavelId = this.filtroResponsavelId === idResponsavel ? null : idResponsavel;
  }
  ...
  tarefasFiltradas(): TarefaResponseDTO[] {
    const pesquisa = this.normalizarTexto(this.filtroPesquisa);

    return this.tarefas.filter((tarefa) => {
      const passaProjeto = !this.filtroProjetoId || tarefa.idProjeto === this.filtroProjetoId;
      const passaResponsavel =
        !this.filtroResponsavelId ||
        tarefa.idResponsavelUsuarioEmpresa === this.filtroResponsavelId;
      const passaDificuldade =
        !this.filtroDificuldade || tarefa.dificuldade === this.filtroDificuldade;
      const passaPrioridade = !this.filtroPrioridade || tarefa.prioridade === this.filtroPrioridade;
      const passaStatus = !this.filtroStatus || tarefa.status === this.filtroStatus;

      const passaConclusao =
        this.filtroConclusao === 'todas' ||
        (this.filtroConclusao === 'somente-concluidas' && tarefa.status === 'concluida') ||
        (this.filtroConclusao === 'ocultar-concluidas' && tarefa.status !== 'concluida');

      const textoTarefa = this.normalizarTexto(`${tarefa.titulo} ${tarefa.descricao ?? ''}`);
      const passaPesquisa = !pesquisa || textoTarefa.includes(pesquisa);
  ...
  trackByTarefaId(_: number, tarefa: TarefaResponseDTO): number {
    return tarefa.idTarefa;
  }

  quantidadeTarefasPorUsuario(idResponsavel: number): number {
    return this.tarefas
      .filter((tarefa) => !this.filtroProjetoId || tarefa.idProjeto === this.filtroProjetoId)
      .filter((tarefa) => tarefa.idResponsavelUsuarioEmpresa === idResponsavel).length;
  }
    ...
    if (this.modoFormulario === 'criar') {
      ...
      this.http.post<TarefaResponseDTO>(this.apiTarefas, payload).subscribe({
        next: (novaTarefa) => {
          ...
          this.filtroProjetoId = novaTarefa.idProjeto ?? this.filtroProjetoId;
          this.salvando = false;
        },
        error: (erro) => {
  ...
  moverTarefa(event: CdkDragDrop<TarefaResponseDTO[]>, novoStatus: StatusTarefa): void {
    ...
    queueMicrotask(() => {
      this.atualizarStatusLocal(idTarefa, novoStatus);

      this.http
        .patch<TarefaResponseDTO>(`${this.apiTarefas}/${idTarefa}/status`, {
          status: novoStatus,
        })
        .subscribe({
          next: (tarefaAtualizada) => {
          ...
          error: (erro) => {
            ...
            this.atualizarStatusLocal(idTarefa, statusAnterior);
  ...
  private atualizarStatusLocal(idTarefa: number, status: StatusTarefa): void {
    this.tarefas = this.tarefas.map((item) =>
      item.idTarefa === idTarefa ? { ...item, status } : item,
    );
  }
  ...
  labelPrioridade(prioridade: PrioridadeTarefa | '' | null | undefined): string {
    if (!prioridade) {
      return 'Todas';
    }
  ...
  labelDificuldade(dificuldade: DificuldadeTarefa | '' | null | undefined): string {
    if (!dificuldade) {
      return 'Todas';
    }
  ...
  labelStatus(status: StatusTarefa | '' | null | undefined): string {
    if (!status) {
      return 'Todos';
    }
  ...
  private novoFormulario(): TarefaForm {
    return {
      ...
      idProjeto: this.filtroProjetoId ?? this.projetoPadrao.id,
  ...
  private montarPayload() {
    ...
    const idEmpresa =
      this.extrairId(projetoSelecionado.idEmpresa, ['idEmpresa', 'id']) ?? this.empresaPadraoId;

    const idProjeto =
      this.extrairId(this.formTarefa.idProjeto, ['idProjeto', 'id']) ?? this.projetoPadrao.id;

    const idResponsavelUsuarioEmpresa = this.extrairId(
      this.formTarefa.idResponsavelUsuarioEmpresa,
      ['idResponsavelUsuarioEmpresa', 'idUsuarioEmpresa', 'id'],
    );

    const payload = {
      idEmpresa,
      idProjeto,
      idResponsavelUsuarioEmpresa,
      ...
      horasEstimadas: Number(this.formTarefa.horasEstimadas ?? 0),
      prazo: this.converterPrazoParaBack(this.formTarefa.prazo),
    };

    console.log('Payload enviado para criar/editar tarefa:', payload);

    return payload;
  }

  private extrairId(valor: any, campos: string[] = []): number | null {
    ...
    if (typeof valor === 'object') {
      for (const campo of campos) {
        if (valor[campo] !== null && valor[campo] !== undefined) {
          return this.extrairId(valor[campo], campos);
        }
      }
    }
